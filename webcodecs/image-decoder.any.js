...
function testFourColorsDecode(filename, mimeType, options = {}) {
  ...
  return fetch(filename)
      .then(response => {
        ...
        return decoder.decode({preferYuv: !!options.yuvFormat});
      })
      .then(result => {
        ...
        let top_right =
            ctx.getImageData(result.image.displayWidth - 1, 0, 1, 1);
        let bottom_left =
            ctx.getImageData(0, result.image.displayHeight - 1, 1, 1);
